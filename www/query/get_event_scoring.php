<?php
    ...
    if (strlen($eventId) > 0) {
        ...
        $q3 = 'SELECT s.*, c.first_name AS competitor_first_name, c.last_name AS competitor_last_name, g.name AS group_name'
            ...
            . ' LEFT OUTER JOIN cmat_annual.competitor c ON (s.competitor_id = c.competitor_id AND c.cmat_year = ' . $CMAT_YEAR . ')'
            . ' LEFT OUTER JOIN cmat_annual."group" g ON (s.group_id = g.group_id AND g.cmat_year = ' . $CMAT_YEAR . ')'
            . ' WHERE s.form_blowout_id = fb.form_blowout_id'
            . " AND fb.event_id = $eventId"
            . " AND fb.cmat_year = $CMAT_YEAR"
            . ' and fb.cmat_year = s.cmat_year';
