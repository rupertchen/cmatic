<?php
	...
	$q0 = 'SELECT r.registration_id, r.competitor_id, c.cmat_year, fb.form_blowout_id'
        ...
        foreach ($individualScoringList as $k => $v) {
            // Create scoring row
	    Db::query(sprintf($p2, $v['cmat_year'], $v['competitor_id'], $v['form_blowout_id']));
            // Link registration row to new scoring row
            Db::query(sprintf($p3, $v['registration_id']));
	}
        foreach ($groupScoringList as $k => $v) {
            // Create scoring row
            Db::query(sprintf($p4, $v['cmat_year'], $v['group_id'], $v['form_blowout_id']));
            // Link registration row to new scoring row
            Db::query(sprintf($p3, $v['registration_id']));
        }
        Db::query('COMMIT');
